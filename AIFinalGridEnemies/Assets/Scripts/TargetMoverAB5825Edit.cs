...
namespace Pathfinding {
	...
	public class TargetMoverAB5825Edit : MonoBehaviour {
		...
		public void UpdateTargetPosition () {
			...
			if (use2D) {
				...
				if (Physics.Raycast(cam.ScreenPointToRay(Input.mousePosition), out hit, Mathf.Infinity, mask)) {
					//EDIT
					if (hit.collider.GetComponent<MouseHover>().withinWalkableCol == true) //if the walkable area is valid at proper distance
					{
